...
| A DBC string offset of 0 points at the start of the string block | **It means "no string", and must be treated as empty without reading.** Blizzard's files open the block with a NUL so the two cannot collide; a repacked table need not, and then every empty reference reads as whatever string is first. That is how a column of blank pet tokens printed as ten class names on a real client. |
| The first string column in `ChrClasses` is the name | **`PetNameToken` at field 3 comes first.** Anything that searches for "a column holding text" finds the decoy. Identify a `_Lang` field by its shape — sixteen columns of which a single-locale client populates exactly one — not by its content. |
