...
| A custom patch goes in `Data/patch-4.MPQ` | **Not one that edits a DBC.** Every base archive loads *before* every locale archive, and a real client keeps `ChrClasses.dbc` in `Data/enUS/patch-enUS-3.MPQ` — so a base-slot patch is shadowed by the client's own and changes nothing at all. Locale content goes in `Data/<locale>/patch-<locale>-N.MPQ`. The recipe said `Data/patch-4.MPQ` until an end-to-end test wrote the file and read it back. |
| Reading the client's tables back after writing the patch is harmless | **Our archive wins the load order — that is the point of it — so it becomes its own input.** A rebuild would record the *patched* tables as the sources it was built from, every later check would compare a patched table against a hash taken from a patched table, and the ledger would be self-consistent and wrong. `recipe::source_order` removes our own output before reading. |
| `url.starts_with("http://127.0.0.1")` means loopback | **It is also true of `http://127.0.0.1.evil.example/` and `http://127.0.0.1:80@evil.example/`**, which are somebody else's server, fetched over plaintext on the strength of looking local. Parse the authority, reject userinfo, compare the host. `manifest::fetchable` is the one definition; `net.rs` calls it rather than keeping a second copy that can drift. |
