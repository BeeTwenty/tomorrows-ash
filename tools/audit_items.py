...
(docs/BODY-TYPES.md): Vanguard = Paladin, Skirmisher = Hunter, Adept = Mage.
...
ARMOR = {"Vanguard": "plate", "Skirmisher": "mail", "Adept": "cloth"}
...
BODY_TYPES = {
    name: (1 << (class_id - 1), CLASS_NAMES[class_id], ARMOR[name])
    for name, class_id in BODY_TYPE_CLASSES.items()
}
REALM_MASK = 0                     # every class that exists on Ashmorrow
for _bit, _name, _armor in BODY_TYPES.values():
    REALM_MASK |= _bit
...
-- (Paladin / Hunter / Mage). AllowableClass is still enforced in
